...
function aggregate_counts(node) {
	if(typeof node.counts != 'undefined'){
		...
		node.counts = aggregate_counts(node.children[0]);
		for(var i = 1; i < node.children.size; i++){
			var next = aggregate_counts(node.children[i]);
			node.counts[i].count += next[i].count;
        }
	...
	if(isAverage){
		for(var i= 0; i < node.counts.length; i++){
			node.counts[i].count /= node.children.length;
		}
	}
}
...
function collect_viewable_nodes(node, node_array)  {
	if(node.view_series === true) node_array.push(node);
    for(var i =0; i<node.children.length; i++){
...
function visualize_time_series(root_node, is_collapsing, selected_node)  {
	...
	for(var node in node_array){
		d3.selectAll('.line').data(node.counts).enter().append('line')
			.attr('points', function(d,i)
			{return d.counts[i][1] + ',' + d.counts[i][2] + ', ';})
			.attr('name', (d)=> d.name)
			.attr('fill', (d)=>d.color)
			.exit()
	}
	...
	var names = d3.selectAll('.text').selectAll('.name');
	for(var node in node_array){
		node.data(node).enter().append('text').attr()
	}
...
function plot_it()  {
	...
	svg.append('g').attr('transform', 'translate('+pad+','+pad+')').attr('id', 'mainplot');
	...
	var countDat = [];
	get_all_count_data(count_tree, countDat);
	var maxCount = Math.max(...countDat);
	var minCount = Math.min(...countDat);
	...
	for(var i = 0; i< count_tree.counts.length; i++){
		dates.push(count_tree.counts[i].date);
	}
	var maxDate = Math.max(...dates);
	var minDate = Math.min(...dates);
	// TODO: compute the x and y scales for the line plots
	var xLineScale = d3.scale.linear.domain([minDate, maxDate]).range([0, svg.width]);
	var yLineScale = d3.scale.linear.domain([minCount, maxCount]).range([0, svg.height]);
	// TODO: setup the line scale
	svg.append()
	// TODO: setup axes from the scales
	svg.append('g').attr('id','topaxis').attr('transform', 'translate(' + '0,' + pad +')')
		.call(d3.axisTop(xScale));
    svg.append('g').attr('id','bottomaxis')
		.attr('transform', 'translate(' + '0,' + minCount+')')
		.call(d3.axisBottom(xScale));
    svg.append('g').attr('id','leftaxis')
		.attr('transform', 'translate(' + '0,' + pad +')')
        .call(d3.axisLeft(yScale));
    svg.append('g').attr('id','rightaxis')
        .attr('transform', 'translate(' + '0,' + maxDate+')')
